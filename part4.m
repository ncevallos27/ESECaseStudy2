...
% out when deaths go down so that means that is around day 135.
firstrunTime = 120; % in days

% constructing the matrix
S_column = [percentNotInfect percentInfect 0 otherDeathPercent 0 0 0]';
I_column = [percentRecoverNoImmunity percentStayInfected percentRecoverImmunity percentInfectDie 0 0 0]';
R_column = [percentRecoverLoseImmunity 0 percentRecoverKeepImmunity 0 0 0 0]';
...
% term.
percentInfect = 0.1;
percentVaccination = 0.001;
percentNotInfect = 1 - (percentInfect + percentVaccination);

percentRecoverNoImmunity = 0.20;
percentRecoverImmunity = 0.0001;
percentInfectDie = 0.00004;
% percentInfected to Vaccinated is always zero as infected people dont get vaccinated until they recover
percentStayInfected = 1-(percentRecoverNoImmunity + percentRecoverImmunity + percentInfectDie);

percentRecoverKeepImmunity = 1 - (percentVaccination);

percentBreakthroughInfection = percentInfect*(vaccineEfficacy);
percentStayVaccinated = 1-percentBreakthroughInfection;

percentDieBI = percentInfectDie*(1-vaccineEfficacy);
percentRecoverVaccineNoImmunity = (percentRecoverNoImmunity)*(vaccineEfficacy);
...
xlabel("days")
title("Mock COVID Data")
legend("new infections per day", "new Infections from model per day")
...
xlabel("days")
title("Mock COVID Data")
legend("Cumulative Deaths", "deaths from model")
